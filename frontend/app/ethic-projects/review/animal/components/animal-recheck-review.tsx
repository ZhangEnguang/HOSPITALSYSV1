...
const getMockReviewRecords = (projectTitle: string): ReviewRecord[] => [
  ...
  },
  {
    ...
    reviewComments: "实验设计存在严重缺陷，动物福利保护措施不足，研究的科学价值与动物使用数量不成比例。建议重新设计实验方案。",
...
export function AnimalRecheckReview({
  ...
  const handleSubmit = async (data: any) => {
    ...
    try {
      ...
      console.log("提交动物伦理复审表单:", submitData)
        ...
        <div className="mb-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              ...
              <div className="flex items-start">
                <span className="text-sm font-medium text-gray-600 w-20 flex-shrink-0 mt-0.5">动物种类:</span>
                <span className="text-sm text-gray-900">{projectData.animalType}</span>
              </div>
              <div className="flex items-start">
                <span className="text-sm font-medium text-gray-600 w-20 flex-shrink-0 mt-0.5">动物数量:</span>
                <span className="text-sm text-gray-900">{projectData.animalCount}</span>
              </div>
              <div className="flex items-start">
                <span className="text-sm font-medium text-gray-600 w-20 flex-shrink-0 mt-0.5">实验执行单位:</span>
