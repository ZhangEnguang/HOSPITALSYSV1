...
export const STEPS: Step[] = [
  {
    id: 0,
    name: "试验阶段信息",
    description: "填写试验阶段和日期信息",
  },
  {
    id: 1,
    name: "实验结果",
    description: "填写实验结果和观察记录",
  },
  {
    id: 2,
    name: "实验过程",
    description: "填写实验方法和步骤",
  },
  {
    id: 3,
    name: "数据分析",
    description: "填写数据分析和结论",
  },
  {
    id: 4,
    name: "完成",
  ...
  const hasStepError = (step: number) => {
    if (step === 0) {
      return Object.keys(validationErrors).some(
        (key) => ["experimentStage", "experimentDate"].includes(key) && validationErrors[key],
      )
    }
    if (step === 1) {
      return Object.keys(validationErrors).some(
        (key) => ["resultSummary"].includes(key) && validationErrors[key],
      )
    }
    return false
  }
