...
const riskAnalysisData = {
  ...
  ethicsComplianceRisk: {
    ...
    commonIssues: [
      { issue: "知情同意书不完整", frequency: 15 },
      { issue: "样本量计算不当", frequency: 12 },
      { issue: "风险评估不足", frequency: 10 },
      { issue: "随访计划不明确", frequency: 8 },
      { issue: "受试者保护措施", frequency: 6 }
    ],
  ...
  participantSafetyRisk: {
    ...
    safetyIndicators: [
      { indicator: "不良事件发生率", baseline: 5, current: 3.2, trend: "down", risk: "低" },
      { indicator: "脱落率", baseline: 15, current: 8, trend: "down", risk: "低" },
      { indicator: "依从性问题", baseline: 10, current: 12, trend: "up", risk: "中" },
      { indicator: "严重不良事件", baseline: 1, current: 0.5, trend: "down", risk: "低" }
    ],
    riskLevels: {
      low: 75,    // 低风险受试者百分比
      medium: 20, // 中风险受试者百分比
  ...
  // 5. 资源与可持续性风险
  sustainabilityRisk: {
    facilityUsage: 82,    // 临床设施使用率
    staffTurnover: 12,    // 人员变更率
    budgetUtilization: 75, // 预算使用率
    resourceEfficiency: [
      { resource: "临床设施", efficiency: 88 },
      { resource: "设备使用", efficiency: 92 },
      { resource: "人员配置", efficiency: 85 },
      { resource: "时间管理", efficiency: 78 }
    ]
  }
}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="border-slate-200 shadow-sm bg-gradient-to-br from-red-50/40 to-slate-50/50">
          ...
          <CardContent>
            <WelfareIndicatorChart data={data.participantSafetyRisk.safetyIndicators} />
            <div className="mt-4 grid grid-cols-3 gap-2 text-xs">
              <div className="text-center p-2 bg-green-50 rounded">
                <div className="text-green-600 font-medium">低风险</div>
                <div className="text-green-800">{data.participantSafetyRisk.riskLevels.low}%</div>
              </div>
              <div className="text-center p-2 bg-yellow-50 rounded">
                <div className="text-yellow-600 font-medium">中风险</div>
                <div className="text-yellow-800">{data.participantSafetyRisk.riskLevels.medium}%</div>
              </div>
              <div className="text-center p-2 bg-red-50 rounded">
                <div className="text-red-600 font-medium">高风险</div>
                <div className="text-red-800">{data.participantSafetyRisk.riskLevels.high}%</div>
              </div>
            </div>
          </CardContent>
        </Card>
