...
function ConsumablesApplicationContent() {
  ...
  // 状态管理
  const [applicationItems, setApplicationItems] = useState(allDemoConsumablesApplicationItems)
  ...
  const filteredApplicationItems = applicationItems
    .filter((item) => {
      ...
      if (
        filterValues.consumableType &&
        filterValues.consumableType !== "" &&
        item.consumableType !== filterValues.consumableType
      ) {
        return false
      }
      ...
      if (filterValues.consumableName && filterValues.consumableName !== "") {
        if (!item.consumableName.toLowerCase().includes(filterValues.consumableName.toLowerCase())) {
          return false
        }
      }
  ...
  const handleBatchDistribute = () => {
    ...
    toast({
      title: "批量发放成功",
      description: `已发放${selectedRows.length}个耗材申领`,
      duration: 3000,
    })
  }
  ...
  const handleRowAction = (actionId: string, item: any) => {
    if (actionId === "view") {
      ...
      router.push(`/laboratory/consumables-application/purchase/${item.id}`)
    } else if (actionId === "record") {
      router.push(`/laboratory/consumables-application/record/${item.id}`)
  ...
  return (
    <div className="space-y-6">
      <DataList
        ...
        onAIAssist={() => router.push("/laboratory/consumables-application/ai-assist")}
        addButtonLabel="新增申领"
        settingsButtonLabel="模板库"
        onOpenSettings={() => router.push("/laboratory/consumables-application/templates")}
        ...
        tableColumns={consumablesApplicationColumns as any}
        tableActions={consumablesApplicationActions}
        cardActions={consumablesApplicationActions}
...
export default function ConsumablesApplicationPage() {
  return (
    <Suspense fallback={<div>加载中...</div>}>
      <ConsumablesApplicationContent />
    </Suspense>
  )
} 