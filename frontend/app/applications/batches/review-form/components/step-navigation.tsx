...
export const STEPS: Step[] = [
  {
    id: 0,
    name: "基本信息",
    description: "填写批次基本信息",
  },
  {
    id: 1,
    name: "评审规则",
    description: "设置批次评审规则",
  },
  {
    id: 2,
    name: "预览确认",
  ...
  const hasStepError = (step: number) => {
    if (step === 0) {
      return Object.keys(validationErrors).some(
        (key) => ["评审计划名称", "评审开始日期", "评审结束日期"].includes(key) && validationErrors[key],
      )
    }
    if (step === 1) {
      return Object.keys(validationErrors).some(
        (key) => ["reviewType", "reviewerIds", "reviewItems"].includes(key) && validationErrors[key],
      )
    }
    return false
  }
