...
export default function AISummary({ data }: AISummaryProps) {
  ...
  const generateAISummary = (isUpdated: boolean) => {
    const awardLevel = data.level || "未知级别"
    const awardStatus = data.status || "未知状态"
    
    if (isUpdated) {
      if (awardStatus === "已获奖") {
        return `《${data.name}》是一项${awardLevel}成果获奖项目，已获得"${data.awardName}"，由${data.venue}颁发。该成果由${data.author.name}主导，共有${data.coAuthors ? data.coAuthors.length + 1 : 1}位完成人员参与。该成果主要内容为：${data.description}。该成果对于${data.project.name}项目具有重要的理论和实践意义，为相关领域的发展提供了创新性解决方案，具有较高的学术价值和应用前景。`
      } else if (awardStatus === "申报中") {
        return `《${data.name}》是一项${awardLevel}成果获奖项目，目前正在申报"${data.awardName}"，将由${data.venue}评审。最新分析显示，该成果竞争力评分达到9.4，共有${data.coAuthors ? data.coAuthors.length + 1 : 1}位完成人员参与。该成果主要内容为：${data.description}。该成果对于${data.project.name}项目具有重要的理论和实践意义，预计将为相关领域的发展提供创新性解决方案。`
      } else {
        return `《${data.name}》是一项${awardLevel}成果获奖项目，${awardStatus}。最新分析显示，该成果具有较高的创新性和应用价值，共有${data.coAuthors ? data.coAuthors.length + 1 : 1}位完成人员参与。该成果主要内容为：${data.description}。该成果对于${data.project.name}项目具有重要的理论和实践意义，预计将为相关领域的发展提供创新性解决方案。`
      }
    } else {
      if (awardStatus === "已获奖") {
        return `《${data.name}》是一项${awardLevel}成果获奖项目，已获得"${data.awardName}"，由${data.venue}颁发。该成果由${data.author.name}主导，共有${data.coAuthors ? data.coAuthors.length + 1 : 1}位完成人员参与。该成果主要内容为：${data.description}`
      } else if (awardStatus === "申报中") {
        return `《${data.name}》是一项${awardLevel}成果获奖项目，目前正在申报"${data.awardName}"，将由${data.venue}评审。共有${data.coAuthors ? data.coAuthors.length + 1 : 1}位完成人员参与。该成果主要内容为：${data.description}`
      } else {
        return `《${data.name}》是一项${awardLevel}成果获奖项目，${awardStatus}。共有${data.coAuthors ? data.coAuthors.length + 1 : 1}位完成人员参与。该成果主要内容为：${data.description}`
      }
    }
  }
  ...
  const generateAIRecommendations = (isUpdated: boolean) => {
    ...
    if (isUpdated) {
      if (awardStatus === "已获奖") {
        return [
          "建议将该获奖成果应用于相关项目实践，提高成果转化率",
          "可考虑基于该成果申请专利保护，提升知识产权价值",
          "建议组织技术推广会，扩大成果影响力并促进产学研合作"
        ]
      } else if (awardStatus === "申报中") {
        return [
          "建议关注评审进度，及时回应评审专家意见",
          "可以准备相关补充材料，以应对可能的答辩要求",
          "同时开展后续研究，为获奖后的推广应用做准备"
        ]
      } else {
        return [
          "建议明确申报重点，突出技术创新点和应用价值",
          "加强与完成人员的沟通协作，提高申报材料质量",
          "制定详细的申报计划，确保研究成果及时获得认可"
        ]
      }
    } else {
      if (awardStatus === "已获奖") {
        ...
        <div className="pl-9 relative">
          {isUpdatingAnalysis && (
            <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-10 flex flex-col items-center justify-center">
              ...
              <div className="text-xs text-slate-500 mt-2">正在处理获奖数据并生成智能洞察...</div>
            </div>
          )}
                ...
                {isAnalysisUpdated ? (
                  <>
                    <div className="flex items-center gap-2 mb-3 text-sm text-amber-600 bg-amber-50 p-2 rounded-md border border-amber-100">
                      ...
                      <span className="font-medium">最新分析已更新 - 检测到获奖详情变化</span>
                    </div>
                    <p>
