...
const levels = ["国际领先", "国际先进", "国内领先", "国内先进", "国家级", "省部级", "市厅级", "校级"]
const types = ["科技成果", "软件成果", "工程技术成果", "理论成果", "应用成果", "社科成果", "艺术成果"]
...
export function StepBasicInfo({ formData, updateFormData, validationErrors }: StepBasicInfoProps) {
  ...
  const handleDateChange = (newDate: Date | undefined) => {
    setDate(newDate)
    if (newDate) {
      updateFormData("evaluationDate", format(newDate, "yyyy-MM-dd"))
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <Input 
            id="projectSource" 
            placeholder="请输入项目来源（选填）" 
            value={formData.projectSource || ""}
            onChange={handleInputChange}
          />
          <p className="text-xs text-muted-foreground">
            鉴定成果所属项目的来源，如国家自然科学基金等
          </p>
        </div>
      </div>
    </div>
