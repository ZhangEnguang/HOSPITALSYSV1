...
import { StepAmountExpense } from "./form-steps/step-amount-expense"
...
// 报销类别
const categories = ["差旅费", "会议费", "材料费", "劳务费", "出版费", "其他费用"]

// 费用类型
const expenseTypes = ["差旅费", "会议费", "材料费", "劳务费", "出版费", "其他费用"]

export function ReimbursementForm({ initialData }: { initialData?: any }) {
  ...
  const updateFormData = (field: string, value: any) => {
    ...
    // 清除验证错误
    if (field === "name" && validationErrors["报销名称"]) {
      setValidationErrors((prev) => {
        const newErrors = { ...prev }
        delete newErrors["报销名称"]
        return newErrors
      })
    } else if (field === "date" && validationErrors["报销日期"]) {
      setValidationErrors((prev) => {
        const newErrors = { ...prev }
        delete newErrors["报销日期"]
        return newErrors
      })
    ...
    } else if (field === "category" && validationErrors["报销类别"]) {
      setValidationErrors((prev) => {
        const newErrors = { ...prev }
        delete newErrors["报销类别"]
        return newErrors
      })
    } else if (field === "amount" && validationErrors["报销金额"]) {
      setValidationErrors((prev) => {
        const newErrors = { ...prev }
        delete newErrors["报销金额"]
        return newErrors
      })
    } else if (field === "expenseType" && validationErrors["费用类型"]) {
      setValidationErrors((prev) => {
        const newErrors = { ...prev }
        delete newErrors["费用类型"]
        return newErrors
      })
    } else if (field === "expenseDate" && validationErrors["费用发生日期"]) {
      setValidationErrors((prev) => {
        const newErrors = { ...prev }
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 0) {
      if (!formData.name) errors["报销名称"] = true
      if (!formData.date) errors["报销日期"] = true
    } else if (step === 1) {
      if (!formData.projectId) errors["所属项目"] = true
      if (!formData.category) errors["报销类别"] = true
    } else if (step === 2) {
      if (!formData.amount) errors["报销金额"] = true
      if (!formData.expenseType) errors["费用类型"] = true
      if (!formData.expenseDate) errors["费用发生日期"] = true
    }
    ...
    <>
      <div className="form-container">
        <div className="flex items-center mb-6">
          ...
          <h1 className="text-2xl font-semibold">{isEditMode ? "编辑经费报销" : "新增经费报销"}</h1>
        </div>
      ...
      <Dialog open={showCompletionDialog} onOpenChange={setShowCompletionDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{isEditMode ? "报销更新成功" : "报销申请提交成功"}</DialogTitle>
            <DialogDescription>
              {isEditMode ? "您的报销申请已成功更新，请选择后续操作" : "您的报销申请已成功提交，请选择后续操作"}
            </DialogDescription>
          </DialogHeader>
