  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 bg-blue-50 p-3 rounded-md">
        ...
        <h3 className="text-base font-medium">确认报销信息</h3>
      </div>
      ...
      <div className="space-y-4">
        <div className="bg-gray-50 p-4 rounded-md">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div>
              <span className="text-gray-500">报销名称：</span>
              <span>{formData.name || "未填写"}</span>
            </div>
            <div>
              <span className="text-gray-500">报销日期：</span>
              <span>{formData.date ? format(new Date(formData.date), "yyyy/MM/dd") : "未选择"}</span>
            </div>
            <div className="col-span-2">
              <span className="text-gray-500">报销描述：</span>
        ...
        <div className="bg-gray-50 p-4 rounded-md">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            ...
            <div>
              <span className="text-gray-500">报销类别：</span>
        ...
        <div className="bg-gray-50 p-4 rounded-md">
          <h4 className="font-medium mb-2">金额与费用信息</h4>
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div>
              <span className="text-gray-500">报销金额：</span>
              <span>{formData.amount ? `${formData.amount} 元` : "未填写"}</span>
            </div>
            <div>
              <span className="text-gray-500">费用类型：</span>
              <span>{formData.expenseType || "未选择"}</span>
            </div>
            <div>
              <span className="text-gray-500">费用发生日期：</span>
              <span>{formData.expenseDate ? format(new Date(formData.expenseDate), "yyyy/MM/dd") : "未选择"}</span>
            </div>
            <div>
              <span className="text-gray-500">票据编号：</span>
              <span>{formData.receiptNumber || "未填写"}</span>
            </div>
          </div>
        </div>
