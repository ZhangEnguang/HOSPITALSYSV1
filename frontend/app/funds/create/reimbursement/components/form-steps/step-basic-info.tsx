...
export function StepBasicInfo({ formData, updateFormData, validationErrors }: StepBasicInfoProps) {
  ...
  const handleDateChange = (newDate: Date | undefined) => {
    ...
    // 清除日期验证错误
    if (newDate && validationErrors["报销日期"]) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <Label htmlFor="name" className="text-sm font-medium flex items-center">
            报销名称 <span className="text-red-500 ml-1">*</span>
          </Label>
          <Input
            id="name"
            name="name"
            ...
            required
            className={cn("h-10", validationErrors["报销名称"] && "border-red-500 focus-visible:ring-red-500")}
          />
          {validationErrors["报销名称"] && (
            <p className="text-sm text-red-500 mt-1">请输入报销名称</p>
          )}
        </div>

        <div className="space-y-2">
          <Label htmlFor="date" className="text-sm font-medium flex items-center">
            报销日期 <span className="text-red-500 ml-1">*</span>
          </Label>
          <Popover>
            <PopoverTrigger asChild>
              <Button
                ...
                className={cn(
                  ...
                  validationErrors["报销日期"] && "border-red-500 focus-visible:ring-red-500"
                )}
              >
          ...
          {validationErrors["报销日期"] && (
            <p className="text-sm text-red-500 mt-1">请选择报销日期</p>
          )}
        </div>
      </div>
