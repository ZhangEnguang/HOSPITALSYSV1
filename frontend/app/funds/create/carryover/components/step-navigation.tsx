...
export const STEPS: Step[] = [
  {
    id: 0,
    name: "基本信息",
    description: "填写结转基本信息",
  },
  {
    id: 1,
    name: "项目与分类",
    description: "选择项目和结转分类",
  },
  {
    id: 2,
    name: "金额与结转",
    description: "填写结转金额和年度",
  },
  {
    id: 3,
    name: "附件上传",
    description: "上传结转相关附件",
  },
  {
    id: 4,
    name: "完成",
  ...
  const hasStepError = (step: number) => {
    if (step === 0) {
      return Object.keys(validationErrors).some(
        (key) => ["结转名称", "结转日期"].includes(key) && validationErrors[key],
      )
    }
    if (step === 1) {
      return Object.keys(validationErrors).some(
        (key) => ["所属项目", "结转类别"].includes(key) && validationErrors[key],
      )
    }
    if (step === 2) {
      return Object.keys(validationErrors).some(
        (key) => ["结转金额", "结转年度(从)", "结转年度(至)", "结转原因"].includes(key) && validationErrors[key],
      )
    }
    return false
  }
