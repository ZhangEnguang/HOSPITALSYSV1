...
// 跟踪审查文件类型定义（包含跟踪审查相关的文件类型）
const fileTypeConfig = {
  // 原有的完整文件类型
  annual_report: { label: "年度报告", icon: <FileText className="h-4 w-4" /> },
  adverse_event: { label: "不良事件报告", icon: <AlertCircle className="h-4 w-4" /> },
  protocol_amendment: { label: "方案修改申请", icon: <FileCheck className="h-4 w-4" /> },
  progress_report: { label: "进展报告", icon: <BarChart3 className="h-4 w-4" /> },
  safety_report: { label: "安全性报告", icon: <FileText className="h-4 w-4" /> },
  completion_report: { label: "项目完成报告", icon: <CheckCircle2 className="h-4 w-4" /> },
  termination_report: { label: "项目终止报告", icon: <XCircle className="h-4 w-4" /> },
  violation_report: { label: "方案偏离报告", icon: <AlertCircle className="h-4 w-4" /> },
  follow_up: { label: "随访记录", icon: <Calendar className="h-4 w-4" /> },
  monitoring: { label: "监察报告", icon: <Eye className="h-4 w-4" /> },
  
  // 数据中使用的简短文件类型（映射到对应的图标）
  report: { label: "报告文件", icon: <FileText className="h-4 w-4 text-blue-600" /> },
  analysis: { label: "分析报告", icon: <BarChart3 className="h-4 w-4 text-green-600" /> },
  safety: { label: "安全性报告", icon: <AlertCircle className="h-4 w-4 text-red-600" /> },
  protocol: { label: "方案文件", icon: <FileCheck className="h-4 w-4 text-purple-600" /> },
  statistics: { label: "统计文件", icon: <BarChart3 className="h-4 w-4 text-indigo-600" /> },
  deviation: { label: "偏离报告", icon: <AlertCircle className="h-4 w-4 text-orange-600" /> },
  explanation: { label: "说明文件", icon: <FileText className="h-4 w-4 text-gray-600" /> },
  assessment: { label: "评估报告", icon: <CheckCircle2 className="h-4 w-4 text-teal-600" /> },
  review: { label: "审查文件", icon: <Eye className="h-4 w-4 text-cyan-600" /> },
  completion: { label: "完成报告", icon: <CheckCircle2 className="h-4 w-4 text-green-600" /> },
  termination: { label: "终止申请", icon: <XCircle className="h-4 w-4 text-red-600" /> },
  application: { label: "申请文件", icon: <FileSignature className="h-4 w-4 text-blue-600" /> },
  consent: { label: "知情同意", icon: <FileCheck className="h-4 w-4 text-indigo-600" /> },
  criteria: { label: "标准文件", icon: <FileText className="h-4 w-4 text-purple-600" /> },
  
  // 默认类型
  other: { label: "其他文件", icon: <FileText className="h-4 w-4 text-slate-600" /> }
}
...
export default function ReviewFilesTab({ project }: ReviewFilesTabProps) {
  ...
  const generateProjectFiles = (): ReviewFile[] => {
    ...
    return project.files.map((file: any, index: number) => ({
      ...
      uploadUser: project.leader || "项目负责人",
      fileSize: file.size,
      status: file.status === "审核通过" ? "approved" as const : 
              file.status === "待审核" ? "pending" as const :
              file.status === "需修改" ? "rejected" as const : "pending" as const,
      version: "1.0",
      reviewComments: file.status === "审核通过" ? "文件审核通过，符合要求" : 
                     file.status === "需修改" ? "文件需要修改，请按意见完善" : "",
      reviewDate: file.status === "审核通过" ? project.approvedAt?.split('T')[0] || "" : "",
      description: `${project.projectType}跟踪审查相关文件`
    }))
  }
  ...
  const handleFileUpload = () => {
    ...
    const newFile: ReviewFile = {
      ...
      uploadUser: project?.leader || "当前用户",
      fileSize: "0MB",
      status: "pending",
      version: "1.0",
      reviewComments: "",
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base font-semibold">跟踪审查文件</CardTitle>
                <CardDescription className="text-slate-500 text-sm">
                  项目跟踪监督阶段，包含年度报告、不良事件报告、进展报告等跟踪文件
                </CardDescription>
              </div>
            </div>
        ...
        <CardContent className="pt-0">
          {filteredFiles.length === 0 ? (
            <div className="text-center py-8 text-slate-500">
              ...
              <p className="font-medium">暂无跟踪审查文件</p>
