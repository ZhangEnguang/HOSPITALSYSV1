...
export default function ReviewFilesTab({ project }: ReviewFilesTabProps) {
  ...
  const generateProjectFiles = (): ReviewFile[] => {
    ...
    return project.files.map((file: any, index: number) => ({
      ...
      description: `${project.projectType}伦理审查相关文件`
    }))
  }
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base font-semibold">初始审查文件</CardTitle>
                <CardDescription className="text-slate-500 text-sm">
                  首次申请审查，包含项目申请表、实验方案等基础文件
                </CardDescription>
              </div>
            </div>
        ...
        <CardContent className="pt-0">
          {filteredFiles.length === 0 ? (
            <div className="text-center py-8 text-slate-500">
              ...
              <p className="font-medium">暂无初始审查文件</p>
