...
import { PlusCircle, Settings, Sparkles, CheckCircle, Trash2 } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import {
  ...
  cardActions,
  filterCategories,
} from "./config/meeting-setup-config"
import { useToast } from "@/components/ui/use-toast"
import { SeniorFilterDTO } from "@/components/data-management/data-list-advanced-filter"
import MeetingSetupCard from "./components/meeting-setup-card"
...
function MeetingSetupContent() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<Record<string, boolean>>({
    ...
    venue: true,
    organizer: true,
    quickReviewCount: true,
    meetingReviewCount: true,
    status: true,
    actions: true,
  })
  ...
  const handleSearch = (value: string) => {
    ...
    const filtered = meetingSetupItems.filter(item => 
      item.title.toLowerCase().includes(value.toLowerCase()) ||
      item.description.toLowerCase().includes(value.toLowerCase()) ||
      item.meetingId.toLowerCase().includes(value.toLowerCase()) ||
      item.venue?.toLowerCase().includes(value.toLowerCase()) ||
      item.organizer?.name?.toLowerCase().includes(value.toLowerCase())
    )
    setData(filtered)
  ...
  const handleQuickFilterChange = (id: string, value: string) => {
    ...
    // 应用筛选
    let filtered = [...meetingSetupItems]
    
    if (newFilterValues.meetingType && newFilterValues.meetingType !== "全部类型") {
      filtered = filtered.filter(item => item.meetingType === newFilterValues.meetingType)
    }
    ...
    if (newFilterValues.venue && newFilterValues.venue !== "全部场地") {
      filtered = filtered.filter(item => item.venue === newFilterValues.venue)
    }
    ...
    if (newFilterValues.committee && newFilterValues.committee !== "全部委员会") {
      filtered = filtered.filter(item => item.committee === newFilterValues.committee)
    }
  ...
  const handleAddNew = () => {
    router.push("/ethic-review/meeting-setup/create")
  }

  const handleAIAssist = () => {
    toast({
      title: "AI智能填报",
      description: "AI智能填报功能正在开发中，敬请期待！",
    })
  }
  ...
  const handleOpenSettings = () => {
    toast({
      title: "设置",
      description: "设置功能正在开发中，敬请期待！",
    })
  }

  const handleItemClick = (item: any) => {
    router.push(`/ethic-review/meeting-setup/${item.id}`)
  }
  ...
  const handleDeleteExecute = () => {
    if (itemToDelete) {
      ...
      // 如果选中的行包含被删除的项目，也要从选中状态中移除
      if (selectedRows.includes(itemToDelete.id)) {
        setSelectedRows(selectedRows.filter(id => id !== itemToDelete.id))
      }
      
      toast({
        title: "删除成功",
        description: `会议"${itemToDelete.title}"已删除`,
      })
    }
  ...
  const batchActions = [
    {
      ...
      onClick: () => {
        toast({
          title: "批量发布",
          description: `已选择 ${selectedRows.length} 个会议进行批量发布`,
        })
      },
    },
    {
      ...
      onClick: () => {
        toast({
          title: "批量删除",
          description: `已选择 ${selectedRows.length} 个会议进行批量删除`,
        })
      },
    },
  ]
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              您确定要删除会议"{itemToDelete?.title}"吗？此操作不可撤销。
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
