  ...
  const getProjectDetail = () => {
    ...
    if (searchId === "4") {
      ...
      try {
        ...
        const project4 = meetingReviewItems.find((p: any) => p.id === "4");
    ...
    // 尝试从meetingReviewItems中查找
    try {
      const { meetingReviewItems } = require("../data/meeting-review-demo-data");
      console.log("尝试从meetingReviewItems查找项目，可用项目数:", meetingReviewItems.length);
      ...
      if (listProject) {
        console.log("在meetingReviewItems中找到项目:", listProject.id, listProject.name);
        ...
        const detailProject = {
          ...
          meetingDate: listProject.meetingDate || "未安排",
        ...
        // 记录所有可用项目ID以便调试
        console.log("可用的项目ID列表:", meetingReviewItems.map((p: any) => p.id).join(", "));
        return null;
      }
    } catch (error) {
      console.error("无法加载或处理会议审查项目数据:", error);
      return null;
    }
  };
  ...
  const getDetailFields = () => {
    ...
    const baseFields = [
      ...
      },
      {
        id: "meetingDate",
        label: "会议日期",
        value: currentProject.meetingDate || "未安排",
        icon: <Calendar className="h-4 w-4 text-gray-400" />,
      }
    ];
