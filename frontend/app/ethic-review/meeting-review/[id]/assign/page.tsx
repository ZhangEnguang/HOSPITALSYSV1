...
export default function AssignExpertsPage({ params }: { params: { id: string } }) {
  ...
  const getProjectDetail = () => {
    ...
    try {
      const { meetingReviewItems } = require("../../data/meeting-review-demo-data");
      console.log("分配专家页面 - 尝试从meetingReviewItems查找项目，可用项目数:", meetingReviewItems.length);
      
      console.log("分配专家页面 - 可用项目ID列表:", meetingReviewItems.map((p: any) => p.id).join(", "));
      
      // 先尝试直接匹配完整ID
      let listProject = meetingReviewItems.find((p: any) => p.id === searchId);
      console.log("分配专家页面 - 直接匹配查找结果:", listProject ? "已找到" : "未找到");
      
      // 如果没找到，尝试从URL中解析出正确的格式
      if (!listProject) {
        // 处理 mr-2024-001 或 2024-001 格式
        ...
        if (idParts.length >= 2) {
          ...
          // 尝试匹配格式为 mr-YYYY-NNN 的项目
          console.log(`分配专家页面 - 尝试匹配格式: mr-${yearPart}-${numberPart}`);
          listProject = meetingReviewItems.find((p: any) => 
            p.id === `mr-${yearPart}-${numberPart}`
          );
        }
      }
      ...
      if (listProject) {
        console.log("分配专家页面 - 在meetingReviewItems中找到项目:", listProject.id, listProject.name);
        ...
        const detailProject = {
          ...
          aiSummary: "该项目提出了基于CRISPR基因编辑技术的罕见遗传病快速基因诊断体系，拟通过基因组筛查和AI辅助分析提高罕见病诊断效率。项目涉及人类基因组数据和动物模型验证，具有中度伦理风险。\n\n建议审查重点关注：\n(1)知情同意过程及基因信息保密措施\n(2)基因编辑范围的明确限制\n(3)动物实验3R原则落实情况\n\n推荐分配具有分子生物学和医学伦理专业背景的专家进行评审。",
      ...
      console.error("分配专家页面 - 无法加载或处理会议审查项目数据:", error);
      return null;
    }
  };
        ...
        <div className="flex items-start">
          ...
          <div className="ml-3">
            ...
            <p className="text-sm text-slate-600 mt-1">
              您正在为会议审查项目分配审查专家。请在右侧<strong>AI智能推荐</strong>面板中查看系统推荐的专家和工作表，
