...
const SheetPortal = ({ className, children, hideOverlay = false, ...props }: SheetPortalProps) => (
  <SheetPrimitive.Portal className={cn(className)} {...props}>
    <div className="fixed inset-0 z-50 flex">
      {/* 当hideOverlay为true时，使用透明背景 */}
      <SheetPrimitive.Overlay
        className={cn(
          ...
          hideOverlay ? "bg-transparent" : "bg-black/80",
        )}
      />
      {children}
    </div>
...
const SheetHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("flex flex-col space-y-2 text-left justify-start", className)} {...props} />
)
